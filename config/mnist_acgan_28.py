...
# experiment_name = "06152017_each_10000_black_white"
experiment_name = "06192017_each_100_black_white/"
...
# # -------------------------------------------------------------------------
	...
	for k in range(N_train):
		...
		Real_img = cv2.resize(cv2.imread(train_file_list[k], 0), (28, 28)).astype(np.uint8)
	...
	for k in range(N_test):
		Real_img = cv2.resize(cv2.imread(test_file_list[k], 0), (28, 28)).astype(np.uint8)
		# Real_img = Real_img.copy()
		# Real_img[:, :, 0] = (Real_img[:, :, 0] - 103.94)/103.94
		# Real_img[:, :, 1] = (Real_img[:, :, 1] - 116.78)/116.78
		# Real_img[:, :, 2] = (Real_img[:, :, 2] - 123.68)/123.68
